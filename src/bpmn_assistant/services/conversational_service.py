...
class ConversationalService:
    ...
    def respond_to_query(
        ...
        yield from self.llm_facade.stream(prompt, max_tokens=999999, temperature=0.5)
    ...
    def generate_conversation_name(self, message_history: list[dict[str, Any]], process: Optional[list[dict[str, Any]]]) -> str:
        """
        Generate a conversation name based on the message history and BPMN process.
        Args:
            message_history: The message history
            process: The BPMN process
        Returns:
        ...
        prompt = self.prompt_processor.render_template(
            "generate_conversation_name.jinja2", **template_vars
        )

        response = self.llm_facade.call(prompt, max_tokens=20, temperature=0.2)
